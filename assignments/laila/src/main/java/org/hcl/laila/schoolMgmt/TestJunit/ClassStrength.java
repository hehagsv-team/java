	...
	protected String classdetails(QueryClass queclass,ExecuteClass execlass, PreparedStatement prestat,Connection connection,int number,ResultSet resultset, String execute) throws SQLException {
		...
		while(true) {
			m.classname();
           	  ...
           	  method=queclass.query9(); 
       	      prestat=connection.prepareStatement(method);
       		  switch(number) {
       		  case 1: 
           	      prestat.setInt(1,6);
                     resultset = prestat.executeQuery();
                     execute=execlass.execute9thquery(resultset,6);
	                  break;
       		  case 2:
           	      prestat.setInt(1,7);
                     resultset = prestat.executeQuery();
                     execute=execlass.execute9thquery(resultset,7);
	                  break;
       		  case 3:
           	      prestat.setInt(1,8);
                     resultset = prestat.executeQuery();
                     execute=execlass.execute9thquery(resultset,8);
	                  break;
       		  case 4:
           	      prestat.setInt(1,9);
                     resultset = prestat.executeQuery();
                     execute=execlass.execute9thquery(resultset,9);
	                  break;
       		  case 5:
           	      prestat.setInt(1,10);
                     resultset = prestat.executeQuery();
                     execute=execlass.execute9thquery(resultset,10);
	                  break;
       		  case 11:
       			  break;
	              default:
