...
public class ClassTeacher {
	final static Logger logger=Logger.getLogger(ClassTeacher.class);
	public String classdetails(QueryClass queryClass,ExecuteClass e,Connection connection,String execute,Scanner scan,boolean isTest) throws SQLException {
		...
		while(true) {
      	  ...
      	  str=queryClass.ClassTeacherQuery(); 
  	      ps=connection.prepareStatement(str);
  		  switch(number) {
  		  case 1:   			  
      	      ps.setInt(1,6);
                rs = ps.executeQuery();
                execute=e.execute1stquery(rs,6);
                break;
  		  case 2:
      	      ps.setInt(1,7);
                rs = ps.executeQuery();
                execute=e.execute1stquery(rs,7);
                break;
  		  case 3:
      	      ps.setInt(1,8);
                rs = ps.executeQuery();
                execute=e.execute1stquery(rs,8);
                break;
  		  case 4:
      	      ps.setInt(1,9);
                rs = ps.executeQuery();
                execute=e.execute1stquery(rs,9);
                break;
  		  case 5:
      	      ps.setInt(1,10);
                rs = ps.executeQuery();
                execute=e.execute1stquery(rs,10);
                break;
  		  case 11:
  			  break;
            default:
