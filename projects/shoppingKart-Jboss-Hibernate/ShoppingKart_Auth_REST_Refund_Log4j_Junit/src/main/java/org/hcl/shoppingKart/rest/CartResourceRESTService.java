 ...
 */
package org.hcl.shoppingKart.rest;
...
import org.hcl.shoppingKart.model.Member;
...
public class CartResourceRESTService {
	...
	static Logger logger = Logger.getLogger(CartResourceRESTService.class);
    ...
    public List<Object[]> listAllMembers(int maxTableSize) {
        ...
        logger.debug("inside listallitems"+resultList);
        return resultList;
    }
    ...
    public List<UserAccount> listAllUsers() {
        ...
        logger.info("Result is" +Userresults);
        return Userresults;
    }
...
//    	members=(List<UserAccount>) em.createQuery("select m.name from UserAccount m where m.name='" +Gowthami "'").getParameter(name);
	...
	public int listByPriceFindSize(String max, String min, int maxTableSize) {
	 ...
	 public List<Object[]> listAllOrders(String username,int maxTableSize) {
	        ...
	        // Use @SupressWarnings to force IDE to ignore warnings about "genericizing" the results of
	        // this query
	    	logger.debug("Enterd into listAllMembers::Orders...::");
	        @SuppressWarnings("unchecked")
...
//	        }
	 ...
	 {
		 logger.debug("Entered into the service class");
		 logger.debug("inside the updateorder the orderid"+order_id);
		 logger.debug("inside the updateorder the username is"+username);
		 int orderid=Integer.parseInt(order_id);
		 logger.debug("After conversion orderid value is"+orderid);
	Query createQuery = em.createQuery("update Orders ord set ord.payment=1 where ord.Id=?0 AND ord.userName=?1");
		...
		if(count>0)
		{
		    ...
		    logger.info(count +"records updated");
		}
      return count;
...
//	} 
	...
	public List<Orders> selects(String order)
	{
		...
		logger.debug("order value in the select is"+orderid);
        ...
        logger.debug("Result is" +Userresults);
        return Userresults;
	}
