...
import org.hcl.shoppingKart.servlet.LogoutServlet;
...
public class LogoutServletTest 
{
    ...
    @Test
    public void doPost_TestUsernameInSession_ShouldGetUsernameAsNullInLogout()
    {
    ...
    @Test
    public void  doPost_TestUsernameInSession_ShouldGetUsername()
    {
    	when(request.getSession(false)).thenReturn(session);
        when(request.getRequestDispatcher("Login.jsp")).thenReturn(rd);
		when(session.getAttribute("SessionUsername")).thenReturn("Hemadri");
			...
			try {
				LogoutServlet logout =new LogoutServlet();
				logout.doPost(request, response);
				assertEquals("Hemadri",logout.getUsername());
			} catch (ServletException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (IOException e) {
    ...
    @Test
    public void doPost_TestSessionGiven_shouldGetSessionAsNullInLogout()
    {
    	when(request.getSession(false)).thenReturn(session);
      when(request.getRequestDispatcher("Login.jsp")).thenReturn(rd);
//		when(session.getAttribute("SessionUsername")).thenReturn("Hemadri");
